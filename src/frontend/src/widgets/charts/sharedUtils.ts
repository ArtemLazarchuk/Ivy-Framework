...
const formatAsDate = (value: number | string, formatter: string): string | null => {
  try {
...
export const formatTickLabel = (
  ...
  if (kind === "Date") {
    return formatAsDate(value, formatter) ?? String(value);
  }
  ...
  if (/(?:^|[^a-zA-Z])(?:yyyy|yy|MMMM|MMM|MM|dd|d|HH|hh|mm|ss)(?:[^a-zA-Z]|$)/.test(formatter)) {
    const dateResult = formatAsDate(value, formatter);
      ...
      formatter: isVertical
        ? (value: string | number) => {
            const formatted = axis.tickFormatter
              ? formatTickLabel(value, axis.tickFormatter, axis.tickFormatterKind)
        ...
        : (value: number | string) => {
            ...
            if (axis.tickFormatter) {
              formatted = formatTickLabel(value, axis.tickFormatter, axis.tickFormatterKind);
            } else {
